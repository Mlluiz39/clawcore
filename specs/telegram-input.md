...
**Usuario:** MlluizDev interagindo do smartphone para a máquina desktop local através de uma DM do Bot do Telegram, mandando comandos de voz dirigindo o carro pedindo para o agente agir e exigindo receber a resposta de volta em formato de voz (TTS Thalita) para audição rápida sem contato visual no app.
...
- Como acionar o Whisper local a partir do backend Node.js (se MlluizDevClaw ainda for Node). *Decisão pendente: usar bridge FFI, ou Child Process nativo na máquina.*
